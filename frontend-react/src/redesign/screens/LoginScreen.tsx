...
/** Styles on/off des onglets Connexion/Inscription (repris de toggleReg() du proto). */
...
const FIELD_VALUE = 'padding:0 16px 12px;font-size:16px;color:rgba(240,236,230,.5)';
...
/** SCREEN: LOGIN — port fidèle (proto l.325-380). toggleReg() → état React `isRegister`. */
export function LoginScreen({ active, goTo }: { active: ScreenId; goTo: (id: ScreenId) => void }) {
  const [isRegister, setIsRegister] = useState(false);

  return (
    <div className={cn('sc', active === 'login' && 'active')} id="sc-login">
      <div style={css('min-height:100%;display:flex;flex-direction:column;padding:70px 24px 24px')}>
        <div style={css('text-align:center;margin-bottom:40px')}>
          ...
          <div style={css('font-size:26px;font-weight:800;letter-spacing:-0.8px;color:#f0ece6;margin-bottom:6px')}>Bon retour !</div>
          <div style={css('font-size:14px;color:rgba(240,236,230,.48)')}>Connecte-toi pour grimper</div>
        </div>
        ...
        <div style={css('display:grid;grid-template-columns:1fr 1fr;gap:4px;padding:4px;border-radius:16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);margin-bottom:28px')}>
          <div onClick={() => setIsRegister(false)} style={css(isRegister ? TAB_OFF : TAB_ON)}>Connexion</div>
          <div onClick={() => setIsRegister(true)} style={css(isRegister ? TAB_ON : TAB_OFF)}>Inscription</div>
        </div>

        {/* Form */}
        <div style={css('display:flex;flex-direction:column;gap:14px')}>
          <GlassCard style={css(`${isRegister ? 'display:block' : 'display:none'};border-radius:16px;overflow:hidden`)}>
            <div style={css('position:relative;z-index:2;padding:0')}>
              <div style={css(FIELD_LABEL)}>Nom d'utilisateur</div>
              <div style={css(FIELD_VALUE)}>ton_pseudo</div>
            </div>
          </GlassCard>
          <GlassCard style={css('border-radius:16px;overflow:hidden')}>
            <div style={css('position:relative;z-index:2')}>
              ...
              <div style={css(FIELD_VALUE)}>ton@email.com</div>
            </div>
          </GlassCard>
          <GlassCard style={css('border-radius:16px;overflow:hidden')}>
            <div style={css('position:relative;z-index:2;display:flex;align-items:center')}>
              <div style={css('flex:1')}>
                ...
                <div style={css(FIELD_VALUE)}>••••••••</div>
              </div>
              <div style={css('padding-right:16px;color:rgba(240,236,230,.35)')}>
          ...
          <div style={css('text-align:right;margin-top:-6px')}>
            <span onClick={() => goTo('forgot-password')} style={css('font-size:13px;color:#D4A030;font-weight:500;cursor:pointer')}>Mot de passe oublié ?</span>
          </div>
          <div style={css('margin-top:8px;padding:16px;border-radius:9999px;background:linear-gradient(145deg,rgba(212,160,48,.90),rgba(232,184,75,.95));border:1px solid rgba(255,255,255,.25);text-align:center;font-size:16px;font-weight:700;color:#1a0f05;cursor:pointer;box-shadow:0 4px 20px rgba(212,160,48,.35),inset 0 1px 0 rgba(255,255,255,.25)')}>Se connecter</div>
        </div>
        ...
        <div style={css('display:flex;align-items:center;gap:12px;margin:24px 0')}>
          ...
          <span style={css('font-size:12px;color:rgba(240,236,230,.35);font-weight:500')}>ou continuer avec</span>
          <div style={css('flex:1;height:1px;background:rgba(255,255,255,.08)')} />
        </div>

        {/* Social login */}
        <div style={css('display:flex;flex-direction:column;gap:10px')}>
          {/* Apple Sign In — official black button */}
          <div style={css(`${SOCIAL_BTN};background:#000;border:1px solid rgba(255,255,255,.14)`)}>
            <div style={css('position:absolute;top:0;left:20%;right:20%;height:1px;background:linear-gradient(90deg,transparent,rgba(255,255,255,.18),transparent)')} />
          ...
          <div style={css(`${SOCIAL_BTN};background:#fff;border:1px solid rgba(0,0,0,.08);box-shadow:0 2px 8px rgba(0,0,0,.25)`)}>
