...
const SYS = 'text-align:center;margin:6px 0;font-size:11px;color:rgba(240,236,230,.4);font-weight:500';
...
export function ConversationPage() {
  ...
  async function send(e: FormEvent) {
    ...
    try { await sendMessage(id, t); } catch { setText(t); } finally { setSending(false); }
  }
  ...
  async function handleFile(file?: File) {
    ...
    catch { /* échec upload */ } finally { setSending(false); }
  }
  ...
  async function shareSpot(s: PickSpot) {
    ...
    setSending(true);
    try { await sendMessage(id, '', undefined, obj); } catch { /* échec */ } finally { setSending(false); }
  }
  ...
  if (!isAuthenticated) {
    return (
      <PageFrame flush>
        <NavBar><div className="nbi"><div className="back-btn" onClick={() => navigate('/redesign/messages')}><BackChevronIcon width={9} height={15} /> Messages</div></div></NavBar>
    ...
    <PageFrame flush>
      <div style={css('display:flex;flex-direction:column;min-height:100%')}>
        <NavBar style={css('height:102px;flex-shrink:0')}>
          <div className="nbi">
            <div className="back-btn" onClick={() => navigate('/redesign/messages')}><BackChevronIcon width={9} height={15} /> Messages</div>
        ...
        <div style={css('flex:1;padding:8px 16px 16px;display:flex;flex-direction:column;gap:6px')}>
          {list === undefined ? (
            ...
            <div style={css('flex:1;display:flex;align-items:center;justify-content:center;color:rgba(240,236,230,.4);font-size:14px;text-align:center;padding:40px 20px')}>Aucun message. Dis bonjour 👋</div>
          ) : (
            list.map((m) => {
        ...
        <div style={css('position:sticky;bottom:0;padding:10px 16px 14px;background:linear-gradient(to top,rgba(10,7,4,.95) 60%,transparent);flex-shrink:0')}>
          ...
          {menuOpen && (
            <div className="g lg-pop" style={css('position:absolute;bottom:72px;left:16px;width:200px;border-radius:16px;overflow:hidden;box-shadow:0 8px 28px rgba(0,0,0,.5)')}>
              <div style={css('position:relative;z-index:2')}>
                <div style={css(`${MENU_ROW};border-bottom:1px solid rgba(255,255,255,.06)`)} onClick={() => photoRef.current?.click()}><span>📷</span> Photo</div>
                <div style={css(`${MENU_ROW};border-bottom:1px solid rgba(255,255,255,.06)`)} onClick={() => openSpotPicker()}><span>📍</span> Partager un spot</div>
                <div style={css(MENU_ROW)} onClick={() => fileRef.current?.click()}><span>📎</span> Fichier</div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={send} style={css('display:flex;align-items:center;gap:10px')}>
            ...
            <div style={css('flex:1;display:flex;align-items:center;background:rgba(255,255,255,.07);backdrop-filter:blur(24px) saturate(1.5);-webkit-backdrop-filter:blur(24px) saturate(1.5);border:1px solid rgba(255,255,255,.11);border-radius:9999px;padding:10px 16px;gap:8px;position:relative;overflow:hidden')}>
              <input value={text} onChange={(e) => setText(e.target.value)} placeholder={sending ? 'Envoi…' : 'Message…'} style={css('font-size:15px;color:#f0ece6;flex:1;background:transparent;border:none;outline:none;font-family:inherit;min-width:0;position:relative;z-index:2')} />
            </div>
            <button type="submit" disabled={sending || !text.trim()} style={css(`width:38px;height:38px;border-radius:50%;background:linear-gradient(145deg,rgba(212,160,48,.88),rgba(232,184,75,.94));display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;box-shadow:0 3px 12px rgba(212,160,48,.30);border:1px solid rgba(255,255,255,.22)${sending || !text.trim() ? ';opacity:.5' : ''}`)}>
          ...
          <div onClick={(e) => e.stopPropagation()} className="g lg-item-in" style={css('border-radius:22px;overflow:hidden;display:flex;flex-direction:column;max-height:100%')}>
            <div style={css('position:relative;z-index:2;display:flex;flex-direction:column;min-height:0')}>
              <div style={css('display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.07)')}>
                ...
                <div onClick={() => setSpotModal(false)} style={css('font-size:14px;color:#D4A030;font-weight:600;cursor:pointer')}>Fermer</div>
              </div>
              <div style={css('padding:12px 16px')}>
                <div style={css('display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10);border-radius:9999px;padding:9px 14px')}>
                  ...
                  <input value={spotQuery} onChange={(e) => setSpotQuery(e.target.value)} placeholder="Chercher un spot…" autoFocus style={css('font-size:14px;color:#f0ece6;flex:1;background:transparent;border:none;outline:none;font-family:inherit;min-width:0')} />
                </div>
              </div>
              <div style={css('overflow-y:auto;padding:0 10px 12px;display:flex;flex-direction:column;gap:4px')}>
                {spots === null ? (
                  <div style={css('padding:24px;text-align:center;color:rgba(240,236,230,.45);font-size:13px')}>Chargement…</div>
                ) : filteredSpots.length === 0 ? (
                  <div style={css('padding:24px;text-align:center;color:rgba(240,236,230,.45);font-size:13px')}>Aucun spot.</div>
                ) : filteredSpots.map((s) => (
                  <div key={s.id} onClick={() => shareSpot(s)} style={css('display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:12px;cursor:pointer')}>
                    <div style={css('width:32px;height:32px;border-radius:9px;background:rgba(212,160,48,.12);border:1px solid rgba(212,160,48,.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px')}>📍</div>
                    <div style={css('min-width:0;flex:1')}>
                      ...
                      <div style={css('font-size:11px;color:rgba(240,236,230,.45)')}>{[SPOT_TYPE_LABEL[s.type], (s.min || s.max) ? `${s.min || '?'}→${s.max || '?'}` : null].filter(Boolean).join(' · ')}</div>
                    </div>
                  </div>
                ))}
              </div>
