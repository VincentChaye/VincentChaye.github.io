...
async function buildCragInfoHTML({ name, lat, lon, source }) {
  ...
  if (!res) {
    res = await searchWikipedia(name || "Falaise", SEARCH_CONFIG.lang || "fr")
      || await searchWikipedia(name || "Falaise", "en");
  }

  const body = res
    ? `<p style="margin:0 0 6px 0;">${res.excerpt || ""}</p><p style="margin:0;">${res.url ? `<a href="${res.url}" target="_blank" rel="noopener">${res.title}</a>` : res.title}</p>`
